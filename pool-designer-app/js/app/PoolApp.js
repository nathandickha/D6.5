...
// --------------------------------------------------------------
  ...
  _applyOvalInfinityArcCut(side, length, width) {
    this._restoreInfinityEdgeCoping();
    this._restoreInfinityWallVoid();
    const centerAngle = side === 'right' ? 0 : side === 'back' ? Math.PI * 0.5 : side === 'left' ? Math.PI : -Math.PI * 0.5;
    const halfArc = Math.PI * 0.25;
    const a = Math.max(0.3, length * 0.5), b = Math.max(0.3, width * 0.5);
    const inArc = (x, y) => this._angleDistance(Math.atan2(y / b, x / a), centerAngle) <= halfArc + 1e-4;

    const wall = this.poolGroup?.userData?.wallMeshes?.[0];
    if (wall?.geometry?.attributes?.position) {
      const original = wall.geometry;
      const g = original.clone();
      g.computeBoundingBox();
      const top = g.boundingBox.max.z;
      const pos = g.attributes.position;
      for (let i = 0; i < pos.count; i += 1) {
        if (Math.abs(pos.getZ(i) - top) < 1e-4 && inArc(pos.getX(i), pos.getY(i))) pos.setZ(i, top - 0.10);
      }
      pos.needsUpdate = true;
      g.computeVertexNormals();
      wall.geometry = g;
